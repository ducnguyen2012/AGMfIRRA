...
def do_train(start_epoch, args, model, train_loader, evaluator, optimizer,
    ...
    meters = {
        ...
        #! duc code: 
        "cmt_loss": AverageMeter(),
        
        "mim_loss": AverageMeter(),
        "mim_acc": AverageMeter(),
    }
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        for n_iter, batch in enumerate(train_loader):
            ...
            meters['mim_acc'].update(ret.get('mim_acc',0),batch_size)
            #! ------
            meters['img_acc'].update(ret.get('img_acc', 0), batch_size)
            meters['txt_acc'].update(ret.get('txt_acc', 0), batch_size)
            meters['mlm_acc'].update(ret.get('mlm_acc', 0), 1)
            meters['cmt_loss'].update(ret.get('cmt_loss', 0), batch_size)
            optimizer.zero_grad()
            total_loss.backward()
            optimizer.step()
            synchronize()
